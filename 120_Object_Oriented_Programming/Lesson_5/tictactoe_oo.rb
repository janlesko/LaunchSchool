...
class Computer < Player
  ...
  def select_marker(human_marker)
    self.marker = (("A".."Z").to_a - [human_marker]).sample
  end

  def moves(board, human_marker)
    square = find_win_move_for(marker, board)
    square = find_win_move_for(human_marker, board) if !square
    square = take_middle(board) if !square
    square = board.empty_squares.sample if !square
    board[square] = marker
  end

  def find_win_move_for(marker, board)
    square = nil
    Board::WINNING_LINES.each do |line|
      square = detect_win_square(line, board.squares, marker)
      break if square
    end
    square
  end
